...
class WebhookAdminController {
  ...
  async getEventTypesSummary(req: Request, res: Response): Promise<void> {
    try {
      ...
      const summary = await Promise.all(eventSummary.map(async item => {
        ...
        return {
          ...
          processedCount,
          failedCount: item._count.eventType - processedCount,
          successRate: Math.round((processedCount / item._count.eventType) * 100),
        };
      }));
  ...
  async getWebhookHealth(req: Request, res: Response): Promise<void> {
    try {
      ...
      const healthStatus = {
        ...
        successRateLastHour:
          totalRecent > 0 ? Math.round((processedRecent / totalRecent) * 100) : 100,
